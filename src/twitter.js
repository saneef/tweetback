...
class Twitter {
	isOriginalPost(tweet) {
		return (
			!this.isRetweet(tweet) && !this.isMention(tweet) && !this.isReply(tweet)
		);
	}
			...
			.filter((tweet) => {
				return (
					this.isSearchMatch(
						tweet,
						searchObj.term,
						searchObj.caseSensitive,
						searchObj.before
					) &&
					!this.isRetweet(tweet) &&
					(searchObj.includeReplies ||
						(!this.isMention(tweet) && !this.isReply(tweet)))
				);
			})
			.sort(function (a, b) {
	...
	getUrlObject(url) {
		...
		// Links to my tweets
		if (
			displayUrl.startsWith(`https://twitter.com/${metadata.username}/status/`)
		) {
			targetUrl = `/${url.expanded_url.substr(
				`https://twitter.com/${metadata.username}/status/`.length
			)}`;
		}
	...
	async renderFullText(tweet, options) {
		...
		if (tweet.entities) {
			...
			for (let mention of tweet.entities.user_mentions) {
				...
				text = text.replace(
					...
					)}" class="tweet-username">${mention.screen_name}</a>`
				);
			}
		}
						...
						try {
							...
							medias.push(
								`<video muted controls ${
									media.type === "animated_gif" ? "loop" : ""
								} src="${remoteVideoUrl}" poster="${
									imgRef.url
	...
	async renderTweet(tweet, options = {}) {
		...
		return `<li id="${tweet.id_str}" class="tweet${
			...
			this.isReply(tweet) && tweet.in_reply_to_screen_name !== metadata.username
				? " is_reply "
				: ""
		...
		${
			this.isReply(tweet)
				? `<a href="${
						tweet.in_reply_to_screen_name !== metadata.username
							? twitterLink(
			...
			<span class="tweet-metadata">
				...
				${
					!this.isRetweet(tweet)
						? `<a href="/" class="tag tag-naked tag-lite tag-avatar"><img src="${metadata.avatar}" width="52" height="52" alt="${metadata.username}’s avatar" class="tweet-avatar"></a>`
						: ""
				}
				${
					...
					${
						shareCount > 0
							? `<span class="tag tag-lite tag-retweet">♻️ ${this.renderNumber(
									shareCount
							  )}<span class="sr-only"> Retweet${
									shareCount !== "1" ? "s" : ""
							  }</span></span>`
							: ""
					}
					${
						tweet.favorite_count > 0
							? `<span class="tag tag-lite tag-favorite">❤️ ${this.renderNumber(
									tweet.favorite_count
							  )}<span class="sr-only"> Favorite${
									tweet.favorite_count !== "1" ? "s" : ""
							  }</span></span>`
							: ""
					}
				`.trim()
				...
				${
					!this.isRetweet(tweet)
						? `<span class="tag tag-naked tag-lite${
								!options.showSentiment || sentimentValue === 0 ? " sr-only" : ""
						  }">Mood ` +
						  (sentimentValue > 0 ? "+" : "") +
	...
	async getReplies(tweet, direction = "next") {
		if (direction === "next") {
			...
			let replyTweet = await dataSource.getTweetById(
	...
	async getReplyHtml(tweet, direction = "next", tweetOptions = {}) {
		...
		for (let replyTweet of replies) {
			let tweetHtml = await this.renderTweet(
				replyTweet,
				Object.assign({ class: `tweet-${direction}` }, tweetOptions)
			);
			let previousHtml =
				direction === "previous"
					? await this.getReplyHtml(replyTweet, direction, tweetOptions)
					: "";
			let nextHtml =
				direction === "next"
					? await this.getReplyHtml(replyTweet, direction, tweetOptions)
			...
			repliesHtml.push(
				(previousHtml
					? `<ol class="tweets-replies">${previousHtml}</ol>`
					: "") +
					tweetHtml +
	...
	async renderTweetThread(tweet, tweetOptions = {}) {
		let previousAndNextTweetOptions = Object.assign({}, tweetOptions, {
			hidePermalink: false,
		});
		let previousHtml = await this.getReplyHtml(
			tweet,
			"previous",
			previousAndNextTweetOptions
		);
		let nextHtml = await this.getReplyHtml(
			tweet,
			"next",
			previousAndNextTweetOptions
		);
	...
	getMostPopularTweets(tweets, limit = 15, forYear) {
		function sortByShareCount(a, b) {
			let shareCountA =
				(parseInt(a.retweet_count, 10) || 0) +
				(parseInt(a.quote_count, 10) || 0);
			let shareCountB =
				(parseInt(b.retweet_count, 10) || 0) +
				(parseInt(b.quote_count, 10) || 0);
		...
		let topTweets = tweets.filter((tweet) => {
			return (
				!this.isRetweet(tweet) &&
				(!forYear || forYear == tweet.date.getFullYear())
			);
		});
		let top = new Set();
